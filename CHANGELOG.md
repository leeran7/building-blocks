...
- **CI required to merge into `main`** — workflow job names are now the
  ...
  `.github/rulesets/require-ci-on-main.json`. A repo admin must apply it
  with `yarn apply-github-ruleset` (the GitHub App cannot create rulesets).
  Until then, GitHub will still merge red PRs.
