...
describe("reduceHold: pointer press/release", () => {
  ...
  it("maps WASD controls to movement and climb axes", () => {
    let memo = initialHoldMemo();
    memo = reduceHold(memo, { kind: "press", id: "climb" });
    expect(touchInputFromHeld(memo.held).up).toBe(true);

    memo = reduceHold(memo, { kind: "release", id: "climb" });
    memo = reduceHold(memo, { kind: "press", id: "down" });
