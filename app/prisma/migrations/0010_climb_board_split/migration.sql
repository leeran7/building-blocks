...
-- comparable, so they no longer share a ranking.
--
-- Cutover vs insert default (do not conflate):
--   ADD COLUMN DEFAULT 'desktop' backfills existing untagged rows onto Desktop
--   (the original 9:16 contract). ALTER COLUMN SET DEFAULT 'mobile' then
--   changes NEW inserts only (omit-POST / Prisma @default). Historical rows
--   are NOT mobile.
