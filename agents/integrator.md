...
`loop/handoffs/integrator-<ISO-timestamp>.json`. `nextStage`: `null` (integrator is the final stage). Code fixes beyond integration → implementer. Intent conflicts → `blocked`.
